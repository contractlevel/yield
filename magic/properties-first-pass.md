...
- The function `_calculateFee` should return `0` if `s_feeRate` is `0`
...
- The Fee Rate Divisor should always equal `1_000_000`
- The Max Fee Rate should always equal `10_000`
...
### State Transitions
- A contract cannot transition from paused to paused state
- A contract cannot transition from unpaused to unpaused state
...
- The Admin Role Transfer Delay should be `259200` seconds (3 days)

## StrategyAdapter

### Spec Based Properties
- All functions with `onlyYieldPeer` modifier should revert if `msg.sender` is not `i_yieldPeer`
- The immutable `i_yieldPeer` should be set in the constructor and never change
...
- The immutable `i_comet` should never change after deployment
...
- The immutable `i_aavePoolAddressesProvider` should never change after deployment
...
- The USDC Decimals constant should be `1e6`
- The Share Decimals constant should be `1e18`
- The Initial Share Precision should be `SHARE_DECIMALS / USDC_DECIMALS`
- The conversion from USDC to Share should multiply by `INITIAL_SHARE_PRECISION`
- The conversion from Share to USDC should divide by `INITIAL_SHARE_PRECISION`
...
- Share Mint Amount should never be `0` (minimum is `1`)
