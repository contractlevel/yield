...
to_bytes32(0x1b5c7cc8fe5bdc6b8f58c2725f45f3058c0a6cbd608ae2f9db497cbde82d4e6d);
...
rule OnReport_revertsWhen_invalidWorkflow() {
    env e;
    bytes report;

    // valid workflow metadata - vars
    bytes32 workflowId;
    address workflowOwner;
    bytes10 workflowName;

    // valid workflow metadata - vars not zero
    ...
    bytes validMetadata = createWorkflowMetadata(workflowId, workflowName, workflowOwner);

    // valid workflow metadata stored in contract
    require currentContract.s_workflows[workflowId].owner == workflowOwner, "workflow owner should be in storage";
    require currentContract.s_workflows[workflowId].name == workflowName, "workflow name should be in storage";

    // invalid workflow metadata - vars
    bytes32 invalidWorkflowId;
    address invalidWorkflowOwner;
    bytes10 invalidWorkflowName;

    // invalid workflow metadata - vars not equal to valid ones
    require invalidWorkflowId != workflowId, "invalidWorkflowId should be different from workflowId";
    require invalidWorkflowOwner != workflowOwner, "invalidWorkflowOwner should be different from workflowOwner";
    require invalidWorkflowOwner != 0x0, "invalidWorkflowOwner should be non-zero";
    require invalidWorkflowName != workflowName, "invalidWorkflowName should be different from workflowName";
    require invalidWorkflowName != to_bytes10(0), "invalidWorkflowName should be non-zero";

    // invalid workflow metadata not stored in contract
    require currentContract.s_workflows[invalidWorkflowId].owner != invalidWorkflowOwner, "invalid workflow owner should not be in storage";
    require currentContract.s_workflows[invalidWorkflowId].name != invalidWorkflowName, "invalid workflow name should not be in storage";

    bytes invalidMetadata = createWorkflowMetadata(invalidWorkflowId, invalidWorkflowName, invalidWorkflowOwner);

    require e.msg.sender == getKeystoneForwarder(), "msg.sender should be keystone forwarder";
    require e.msg.value == 0, "msg.value should be zero";

    // call with invalid workflow metadata
...
rule setWorkflow_revertsWhen_workflowIdZero() {
    ...
    bytes32 workflowId;
    address workflowOwner;
    string workflowNameRaw;

    require e.msg.value == 0, "msg.value should be zero";
    require e.msg.sender == owner(), "msg.sender should be owner";
    require workflowId == to_bytes32(0x0), "workflowId should be zero";

    setWorkflow@withrevert(e, workflowId, workflowOwner, workflowNameRaw);
    assert lastReverted;
}
...
// @review False positive
rule setWorkflow_revertsWhen_workflowNameEmpty() {
    env e;
    bytes32 workflowId;
    address workflowOwner;
    string workflowNameRaw;
    ...
    require e.msg.value == 0, "msg.value should be zero";
    require e.msg.sender == owner(), "msg.sender should be owner";
    require workflowNameRaw == "", "workflowNameRaw should be empty";
...
// @review rule vacuous
rule setWorkflow_success() {
    env e;
    bytes32 workflowId;
    address workflowOwner;
    string workflowNameRaw;

    require workflowNameRaw == "a", "workflowNameRaw should be non-empty";

    setWorkflow(e, workflowId, workflowOwner, workflowNameRaw);
