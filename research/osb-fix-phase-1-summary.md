...
**Why:** The ERC677 `transferAndCall` mechanism already moves the shares to the peer contract before `onTokenTransfer` fires — so the shares are *held* automatically. We just stopped burning them prematurely. The burn is now deferred to `_handleCCIPWithdrawCallbackChild` after the strategy confirms the withdrawal.
...
- In the parent-is-strategy branch: performs strategy withdrawal, decrements `s_totalShares`, emits `ShareBurnUpdate`, and sends `WithdrawCallbackChild` to the withdraw chain. State update happens *after* confirmed withdrawal.
...
The following mid-flow unit tests are still needed:

**`test/unit/parentPeer/ParentWithdraw.t.sol`:**
- `test_yield_parent_onTokenTransfer_sharesHeldByPeer_notBurned` — after `transferAndCall` when strategy is on a child, assert: `baseParentPeer` holds shares, `getTotalShares()` unchanged, total supply unchanged.
- `test_yield_parent_withdraw_totalSharesNotDecrementedBeforeStrategyConfirms` — Scenario E (strategy on opt child): after routing base→opt (`WithdrawToStrategy`) but before routing opt→base (`WithdrawCallbackParent`), assert `getTotalShares()` equals pre-withdrawal value.

These tests check state between CCIP hops, which is the only way to verify the deferred-update guarantee in fork tests.
