...
contract ProxyHandler is Test {
    ...
    constructor(
        ...
        ProxyHandler.SystemRoles memory _systemRoles
    ) {
        share = _share;
        usdc = _usdc;
        rebalancer = _rebalancer;
    ...
    function rebalance(uint256 chainSeed, uint256 protocolSeed) public virtual {
        ...
        // temp fix in case strategy is already and will revert
        ParentPeer parent = ParentPeer(address(peers[1]));
        IYieldPeer.Strategy memory strategy = parent.getStrategy();
        if (strategy.chainSelector == targetChain && strategy.protocolId == protocolId) {
            if (protocolId == AAVE_V3_PROTOCOL_ID) protocolId = COMPOUND_V3_PROTOCOL_ID;
            else protocolId = AAVE_V3_PROTOCOL_ID;
        }
    ...
    function _setFeeRate(uint256 newFeeRate) internal {
        ...
        address feeRateSetter = systemRoles.feeRateSetter;
    ...
    function _setCCIPGasLimit(uint256 ccipGasLimit) internal {
        ...
        ParentPeer parent = ParentPeer(address(peers[PARENT_CHAIN_SELECTOR]));
        address crossChainAdmin = systemRoles.crossChainAdmin;
    ...
    function _togglePause() internal {
        ...
        if (!globalGhost.paused) {
            address pauser = systemRoles.emergencyPauser;
            vm.startPrank(pauser);
            for (uint256 i = 0; i < activeSelectors.length; i++) {
                Pausable(address(peers[activeSelectors[i]])).pause();
            }
            vm.stopPrank();
            globalGhost.paused = true;
        } else {
            address unpauser = systemRoles.emergencyUnpauser;
            vm.startPrank(unpauser);
            for (uint256 i = 0; i < activeSelectors.length; i++) {
                Pausable(address(peers[activeSelectors[i]])).unpause();
            }
            vm.stopPrank();
    ...
    function _triggerRebalancerUpgrade(uint256 seed) internal {
        ...
        address newImpl = address(new MockUpgradeRebalancer(rebalancer.getParentPeer()));
        _verifyImplementationLocked(newImpl);
