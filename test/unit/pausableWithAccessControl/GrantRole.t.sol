...
contract GrantRoleTest is BaseTest {
    ...
    function test_yield_pausableWithAccessControlRebalancer_grantRole_updatesRoleStorage() public {
        ...
        */
        ...
        assertEq(baseRebalancer.hasRole(Roles.CONFIG_ADMIN_ROLE, newConfigAdmin), true); /// -------------------- @dev Check hasRole in _roles mapping updated
        assertEq(baseRebalancer.getRoleMember(Roles.CONFIG_ADMIN_ROLE, ROLE_MEMBER_INDEX), newConfigAdmin); /// - @dev Check s_roleMembers updated
        assertEq(baseRebalancer.getRoleMemberCount(Roles.CONFIG_ADMIN_ROLE), ROLE_MEMBER_COUNT); /// ------------ @dev Check s_roleMembers updated
        assertEq(baseRebalancer.getRoleMembers(Roles.CONFIG_ADMIN_ROLE), expectedConfigAdminRoleMembers); /// --- @dev Check s_roleMembers updated
    }
