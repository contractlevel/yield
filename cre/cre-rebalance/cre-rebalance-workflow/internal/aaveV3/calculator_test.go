...
func Test_convertAPRToAPY_typicalAPR(t *testing.T) {
	...
	require.Less(t, apy, 0.06)             // But not too much higher
...
func Test_convertAPRToAPY_negativeAPR(t *testing.T) {
	// Test negative APR (should return 0.0, no error)
	aprRat := big.NewRat(-1, 100) // -1%
	apy, err := convertAPRToAPY(aprRat)
	require.NoError(t, err)
	require.Equal(t, 0.0, apy)
}

func Test_convertAPRToAPY_extremeValuesToTriggerNaNInf(t *testing.T) {
	// Try to trigger NaN/Inf by using values that might cause floating point overflow
	// Using a value just under the 1000% limit but with extreme precision
	// 999.999% APR = 9.99999
	aprRat := big.NewRat(999999, 100000) // 9.99999 = 999.999%
	apy, err := convertAPRToAPY(aprRat)
	// This should either work or trigger the NaN/Inf check
	if err != nil {
		require.ErrorContains(t, err, "APR exceeds 1000%")
	} else {
		// If it succeeds, verify it's a valid number
		require.GreaterOrEqual(t, apy, 0.0, "APY should be >= 0")
		require.False(t, apy != apy, "APY should not be NaN") // NaN != NaN is true
	}
}

func Test_convertAPRToAPY_NaNInfHandling(t *testing.T) {
	// This test attempts to trigger NaN/Inf by using extreme values
	// However, the 1000% check will catch most cases before NaN/Inf occurs
	// We test that the function handles edge cases gracefully

	// Test with a value that might cause issues (though unlikely to reach NaN/Inf)
	// Using maximum precision big.Rat that's just under 1000%
	maxValidAPR := big.NewRat(99999, 10000) // 9.9999 = 999.99%
	apy, err := convertAPRToAPY(maxValidAPR)

	// Should either succeed or error before NaN/Inf
	if err != nil {
		require.ErrorContains(t, err, "APR exceeds 1000%")
		require.Equal(t, 0.0, apy)
	} else {
		// If it succeeds, verify it's a valid number (not NaN or Inf)
...
func Test_CalculateAPYFromContract_nilLiquidityRate(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)

	// Test nil liquidityRate (shouldn't happen with generated bindings, but defensive check)
	mockStrategy := &mockStrategyContract{
		calculateInterestRatesFunc: func(_ cre.Runtime, _ default_reserve_interest_rate_strategy_v2.CalculateInterestRatesInput, _ *big.Int) cre.Promise[default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput] {
			return cre.PromiseFromResult(default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput{
				Arg0: nil, // nil liquidityRate
				Arg1: big.NewInt(0),
			}, nil)
		},
	}
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "contract returned nil liquidityRate")
	require.Equal(t, 0.0, apy)
}

//@review redundant check in calculator.go
func Test_CalculateAPYFromContract_negativeLiquidityRate(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)

	// Test negative liquidityRate (shouldn't happen, but defensive check)
	negativeRateRAY := new(big.Int).Neg(big.NewInt(1))

	mockStrategy := &mockStrategyContract{
		calculateInterestRatesFunc: func(_ cre.Runtime, _ default_reserve_interest_rate_strategy_v2.CalculateInterestRatesInput, _ *big.Int) cre.Promise[default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput] {
			return cre.PromiseFromResult(default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput{
				Arg0: negativeRateRAY,
				Arg1: big.NewInt(0),
			}, nil)
		},
	}
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "contract returned negative liquidityRate")
	require.Equal(t, 0.0, apy)
}
...
//redundant
func Test_CalculateAPYFromContract_nilLiquidityAdded(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)

	mockStrategy := &mockStrategyContract{
		calculateInterestRatesFunc: func(_ cre.Runtime, _ default_reserve_interest_rate_strategy_v2.CalculateInterestRatesInput, _ *big.Int) cre.Promise[default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput] {
			return cre.PromiseFromResult(default_reserve_interest_rate_strategy_v2.CalculateInterestRatesOutput{
				Arg0: big.NewInt(1000000),
				Arg1: big.NewInt(0),
			}, nil)
		},
	}
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "LiquidityAdded cannot be nil")
	require.Equal(t, 0.0, apy)
}

func Test_CalculateAPYFromContract_nilLiquidityTaken(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "LiquidityTaken cannot be nil")
	require.Equal(t, 0.0, apy)
}

func Test_CalculateAPYFromContract_nilTotalDebt(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "TotalDebt cannot be nil")
	require.Equal(t, 0.0, apy)
}

func Test_CalculateAPYFromContract_nilReserveFactor(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "ReserveFactor cannot be nil")
	require.Equal(t, 0.0, apy)
}

func Test_CalculateAPYFromContract_nilVirtualUnderlyingBalance(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "VirtualUnderlyingBalance cannot be nil")
	require.Equal(t, 0.0, apy)
}

func Test_CalculateAPYFromContract_zeroAddress(t *testing.T) {
	runtime := testutils.NewRuntime(t, nil)
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "Reserve cannot be zero address")
	require.Equal(t, 0.0, apy)
}
